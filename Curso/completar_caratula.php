<html>
	<head>
		<script>
			...
			{     
				$(".contenedor_cursos").load( "./Lista_Cursos.php", {id: $("#id_programa").val()});				
	...
	<?php
		...
		$id = $_POST["id"];
		$conexion = new Querys();
		$curso = $conexion->Consultas("SELECT Institucion.Nombre as Institucion, ID_Institucion, Unidad.Nombre as Unidad, ID_Unidad, Nombre_Programa, ID_Programa, ID_Programa_Unidad FROM Institucion, Unidad, Programa_Unidad, Programa_Academico WHERE ID_Institucion = FK_Institucion AND ID_Unidad = FK_Unidad AND FK_Programa = ID_Programa AND ID_Programa_Unidad = ".$id);
		$curso = $curso[0];
	?>
	<body>
		<input type="hidden" value="<?php echo $curso["ID_Programa_Unidad"];?>" id="id_programa">
					...
					<div class="facilitar_cabecera"><!--facility-header-->
								...
								<tbody>
									<tr>
										<td valign="bottom" height="80px;">
											<h1>
												<a href="../Programa_Academico/Informacion.php?i=<?php echo $curso["ID_Programa"];?>&n=<?php echo $curso["Nombre_Programa"];?>"><?php echo $curso["Nombre_Programa"]; ?></a>
											</h1>
										</td>
									</tr>
									<tr>
										<td>
											<div class="meta">
												<a href="../Institucion/Informacion.php?i=<?php echo $curso["ID_Institucion"];?>&n=<?php echo $curso["Institucion"];?>"><?php echo $curso["Institucion"]; ?></a>
											</div>
										</td>
									</tr>
									<tr>
										<td>
											<div class="meta">
												<a href="../Unidad/?i=<?php echo $curso["ID_Unidad"];?>&n=<?php echo $curso["Unidad"];?>"><?php echo $curso["Unidad"]; ?></a>
											</div>
										</td>
									</tr>
								</tbody>
						...
						<div class="facilitar_cabecera_acciones rf"><!--facility-header-actions rf-->
							<div class="facilitar_accion_primaria contenedor_js"><!--facility-primary-action js-widgetcontainer-->
								<a class="boton boton_promover boton_ancho_completo boton_largo boton_invitar" value="<?php echo $curso["ID_Programa_Unidad"];?>"><!--btn btn-promote btn-fullwidth btn-large btn-invite-->Agregar Curso</a>
							</div>					
						</div>
						<?php
							}
