<html>
				...
				$(".boton_invitar").click(function(){
					...
					$.post("../popup/popup.php", function(data){
						...
						$.post("./Agregar.php", function(data){
							...
							$(".mensaje").hide();
							$("#id_programa").val(id);
							$.post("../Scripts/consulta.php", {opcion: "colegio_programa", id: $("#id_programa").val()}, function(data){
								if(data.coordinador == 1)
									$("#coordinador").hide();
	...
	<?php
		...
		$id = $_POST["id"];
		$conexion = new Querys();
		$colegio = $conexion->Consultas("SELECT Institucion.Nombre as Institucion, ID_Institucion, Unidad.Nombre as Unidad, ID_Unidad, Nombre_Programa, ID_Programa, ID_Programa_Unidad FROM Institucion, Unidad, Programa_Unidad, Programa_Academico WHERE ID_Institucion = FK_Institucion AND ID_Unidad = FK_Unidad AND FK_Programa = ID_Programa AND ID_Programa_Unidad = ".$id);
		$colegio = $colegio[0];
	?>
	<body>
		<input type="hidden" value="<?php echo $colegio["ID_Programa_Unidad"];?>" id="id_programa">
					...
					<div class="facilitar_cabecera"><!--facility-header-->
								...
								<tbody>
									<tr>
										<td valign="bottom" height="80px;">
											<h1>
												<a href="../Programa_Academico/Informacion.php?i=<?php echo $colegio["ID_Programa"];?>&n=<?php echo $colegio["Nombre_Programa"];?>"><?php echo $colegio["Nombre_Programa"]; ?></a>
											</h1>
										</td>
									</tr>
									<tr>
										<td>
											<div class="meta">
												<a href="../Institucion/Informacion.php?i=<?php echo $colegio["ID_Institucion"];?>&n=<?php echo $colegio["Institucion"];?>"><?php echo $colegio["Institucion"]; ?></a>
											</div>
										</td>
									</tr>
									<tr>
										<td>
											<div class="meta">
												<a href="../Unidad/?i=<?php echo $colegio["ID_Unidad"];?>&n=<?php echo $colegio["Unidad"];?>"><?php echo $colegio["Unidad"]; ?></a>
											</div>
										</td>
									</tr>
								</tbody>
						...
						<div class="facilitar_cabecera_acciones rf"><!--facility-header-actions rf-->
							<div class="facilitar_accion_primaria contenedor_js"><!--facility-primary-action js-widgetcontainer-->
								<a class="boton boton_promover boton_ancho_completo boton_largo boton_invitar" value="<?php echo $colegio["ID_Programa_Unidad"];?>"><!--btn btn-promote btn-fullwidth btn-large btn-invite-->Agregar Profesor</a>
							</div>					
						</div>
						<?php
							}
